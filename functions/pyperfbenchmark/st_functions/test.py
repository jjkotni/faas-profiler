...
def functionWorker(tname, allocate_pkey, runner):
    if allocate_pkey:
...
#    runner = pyperf.Runner()
    ...
    runner.bench_command('2to3' + tname, command)
...
#    del runner
    ...
    for i in range(workers):
        tname = 'Worker' + str(i)
        threads.append(mp.Process(target=functionWorker, args=[tname,0,runner], name=tname))
